...
def format_docs_for_planner(
    ...
    if is_retrieval_bundle(bundle_or_context):
        ...
        query_text = bundle_or_context.get("query", "N/A")
...
def get_module_summary(bundle_or_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    avg_similarity = metadata.get("avg_atomic_score", 0) if is_retrieval_bundle(bundle_or_context) else metadata.get("avg_confidence_score", 0)
